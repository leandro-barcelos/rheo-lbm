...
void ui::ImGuiLayer::Init(platform::Window const& window,
  ...
  descriptor_pool_ =
      vk::raii::DescriptorPool(device.LogicalDevice(), descriptor_pool_info);
  ...
  ImGui_ImplGlfw_InitForVulkan(static_cast<GLFWwindow*>(window.NativeHandle()),
                               true);

  auto color_attachment_format =
      static_cast<VkFormat>(swap_chain.SurfaceFormat().format);
  VkPipelineRenderingCreateInfoKHR rendering_info{
      .sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR,
      .colorAttachmentCount = 1,
      .pColorAttachmentFormats = &color_attachment_format};

  min_image_count_ = std::max(2U, swap_chain.ImageCount());
...
void ui::ImGuiLayer::Render(vk::CommandBuffer command_buffer) const {
  if (!initialized_) {
    return;
  }
...
void ui::ImGuiLayer::OnFrameResourcesChanged(std::uint32_t image_count) {
  if (!initialized_) {
    return;
  }

  uint32_t const next_min_image_count = std::max(2U, image_count);
