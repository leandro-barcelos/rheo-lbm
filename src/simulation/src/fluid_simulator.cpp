...
simulation::FluidSimulator::FluidSimulator(
    ...
    : uniform_buffer_data_{} /*, dem_(parameters.dem)*/ {
  ...
  uniform_buffer_data_ = {
      ...
      .elevation_width = parameters.dem->width,
      .elevation_height = parameters.dem->height,
...
void simulation::FluidSimulator::Init(
  ...
  //   bucket_descriptor_allocator_.Init(device, 1,
  //   bucket_descriptor_pool_sizes); CreateBucketPipelines(device);
  ...
  //       graphics::DescriptorAllocator::PoolSize{
  //           .type = vk::DescriptorType::eStorageBuffer, .count = 3}};
  //   density_descriptor_allocator_.Init(device, 1,
  //   density_descriptor_pool_sizes); CreateDensityPipeline(device);
  //   CreateDensityDescriptorSets(device, density_descriptor_allocator_);
  //   CreateDensityCommandBuffers(device, command_pools);

  //   if (dem_ == nullptr || dem_->samples.empty()) {
  //     throw std::runtime_error("[ERROR] Simulation: Missing elevation
  //     samples!");
  //   }

  //   auto elevation_buffers = graphics::BufferAllocator::CreateSSBO(
  //       device, command_pools, dem_->samples, false,
  ...
  //   vel_pos_descriptor_allocator_.Init(device, 1,
  //   vel_pos_descriptor_pool_sizes); CreateVelPosPipeline(device);
...
//       sizeof(Cell) * uniform_buffer_data_.fluid_particle_count);
...
//       vk::raii::CommandBuffers(device.LogicalDevice(),
//       primary_allocate_info);
//   bucket_primary_command_buffer_ =
//   std::move(primary_command_buffers.front());
...
//       sizeof(domain::Elevation) * dem_->samples.size());
